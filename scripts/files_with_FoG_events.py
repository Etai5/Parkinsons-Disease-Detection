""" Given data which have the FoG label column, this script run over all the files and filter
    ...
    a subfolder called "files_with_fog_events" which includes only the files which has at least one FoG event.
"""
...
# Function to process and copy files with at least one FoG event
def process_fog_files(input_directory, output_directory):
    ...
    for file_path in csv_files:
        try:
            ...
            # Check if there is any row where the FoG column has a value of 1
            if (df['FoG'] == 1).any():
                ...
                print(f"No FoG event in file: {file_name}")
...
input_directory = r'D:\\University\\tdcsfog\\fog'
